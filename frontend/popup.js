...
function main() {
  ...
  function showLoginUI() {
    ...
    featureButton.style.display = "block";
  ...
  // Added a Feature Button
  featureButton.addEventListener("click", function () {
    window.location.href =
      "https://github.com/SarthakSri98/inspector-saab-frontend/issues";
  });
  ...
  const jsSnippets = {
    ...
    addDarkMode:
      "A dark mode button at the top right to toggle dark mode when clicked",
    addReadingProgressBar:
      "A progress bar at the top which increases as we scroll to the bottom",
    highlightContent:
      "A method to highlight content on the site as per json returned",
  };
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            ...
            reject(
              response?.error ||
                "Failed to process request in background script."
            );
          }
        }
      );
